...
$$
\tau_{\rm MRX} = \frac{1}{B^2/8\pi} \frac{d}{dt} \left( \frac{B^2}{8\pi} \right)
...
![](figures/mrx_sweet_parker.png){width=600px}
...
![](figures/mrx_petschek.png){width=600px}
...
$$
\left[ \rho \bm{v} \bm{v} - \frac{\bm{B}\bm{B}}{4\pi} \right] = 0
...
$$
where $\delta/L \sim M_{\rm A}^{-1} R_{\rm m}^{-1}$. It now depends only very weakly on $R_{\rm m}$. A fast reconnection with $M_{\rm A} \sim 0.01$ is possible for diverse space plasma applications. However, ...
...
#### Relation to Generalized Ohm's Law
...
Torbert et al. (2018, Science)

![](figures/mms-mrx-summary.png){.absolute height=650 left=50 top=70}
![](figures/mms-mrx-vdf1.png){.absolute height=325 left=650 top=70}
![](figures/mms-mrx-vdf2.png){.absolute height=325 left=650 top=400}
...
![](figures/gem-mrx.png){.absolute width=900 top=200}


- Comparison between different simulation models suggests that the Hall term is the essential physics for fast magnetic reconnection.

---

#### Hall-mediated Magnetic Reconnection

Treumann & Baumjohann (2013, Front. Phys.)

- Hall term does not directly contribute to the dissipation because $\bm{j} \cdot (\bm{j} \times \bm{B}) = 0$.
- However, it potentially enhances reconnection because of the formation of a narrow and fast electron jet in the ion diffusion region that may carry magnetic flux away from the diffusion region efficiently.
- On the other hand, simulation studies suggest that the Hall term is not alway necessary to achieve fast reconnection.
- The Hall magnetic field (or out-of-plane) component is considered as the signature of magnetic reconnection of in-situ spacecraft measurements in space.

![](figures/hall-mrx-schematic.png){ width=600 style="position: absolute; left: 50px; top: 350px" }
![](figures/hall-mrx-simulation.png){ width=600 style="position: absolute; left: 650px; top: 350px" }
