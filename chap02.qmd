...
![height:500px center](figures/soho.png)

---

### Nonthermal Particle Distribution

- Particle energy distribution in space is not in thermodynamic equilibrium. The energy equipartition theorem is not satisfied in space.
- Power-law distribution is commonly seen both in natural and social systems.

<img src="figures/cosmic-ray-spectrum.png" height="500px" style="position: absolute; left: 100px; top: 180px"/>
<img src="figures/corporate-income.png" height="500px" style="position: absolute; left: 600px; top: 180px"/>
...
![height:450px center](figures/solar-energetic-particle.png)

---

### Cosmic-ray Acceleration at Supernova Remnant Shocks

![height:450px center](figures/supernova-remnant.png)
...
\frac{N(E)}{\tau_{\rm esc}}
$$
with the typical acceleration time $\tau_{\rm acc}$ defined as
$$
\frac{1}{\tau_{\rm acc}} \equiv \frac{1}{E} \frac{dE}{dt}.
$$

A power-law spectrum is obtained in the steady state only when the acceleration time and the escape time both have the same energy dependence:
$$
N(E) = N_0 \left( \frac{E}{E_0} \right)^{-p}
\quad
p \equiv \frac{\tau_{\rm acc}}{\tau_{\rm esc}} + 1
$$

Understanding a particle acceleration mechanism to produce a power-law spectrum is essentially to find a specific physical process that gives the acceleration time $\tau_{\rm acc}$ and the escape time $\tau_{\rm esc}$.
...
The conservation of energy in the rest frame of the obstacle (moving with a speed $V$) indicates that the particle energy gain per collision is given by:
$$
\frac{\Delta E}{E} \sim 2 \frac{V}{c} \cos \theta,
$$
where $\theta$ is the angle between the velocities of the particle and the obstacle. Note that we consider that the particle energy is relativistic $v \sim c$.
...
&=
\frac
	{\int_{-1}^{+1} \frac{\Delta E}{E} \mu (1 + \frac{V}{c} \mu) d \mu}
	{\int_{-1}^{+1} \mu (1 + \frac{V}{c} \mu) d \mu}
	\qquad \mu = \cos \theta
	\\
...
This indicates that the acceleration time is independent of energy. If we *assume* that $\tau_{\rm esc}$ is also independent of energy, the power-law spectrum is obtained with its index given by $q = \tau_{\rm acc}/\tau_{\rm esc} + 1$.
...
1. The original Fermi's idea considering cosmic ray acceleration in interstellar turbulence (molecular clouds with $V \sim 10 \, {\rm km/s}$, $\lambda_{c} \sim 1 \, {\rm pc}$) indicates that $\tau_{\rm acc} \sim 6 \times 10^{16} \sim 2 \times 10^{9} {\rm yrs}$, which is probably too slow (cf., typical p-p collision loss time: $\sim 10^{15} \, {\rm s}$, cosmic age: $\sim 4 \times 10^{17} \, {\rm s}$.)

---

### First-order Fermi Acceleration
The first-order Fermi acceleration has been the standard model for the shock acceleration of cosmic rays. The particles gain energy by repeatedly crossing the shock front back and force.

![](figures/scholer.png){height=400}

*Courtesy by M. Scholer*
...
The energy gain via a single collision with an irregularity is given by $\frac{\Delta E}{E} \approx 2 \frac{V}{c} \cos \theta$, where $V = V_1$ or $V = V_2$ denotes the flow speeds in the upstream and downstream assuming that irregularities are moving with the flow.

Assume that the pitch angle of a particle is $\theta_{+}$ for crossing the shock front from upstream to downstream and $\theta_{-}$ for downstream to upstream. The energy gain per one cycle (i.e., downstream → upstream → downstream) is given by
$$
...
\underbrace{\sin \theta_{+}}_{\textrm{Jcobian}} d \theta_{+} =
...
$$
which is first order in $V/c$ The first-order energy gain remains finite because of $V_1 \neq V_2$ across the shock front. The positive energy gain $\Delta E > 0$ is a consequence of the compression at the hydrodynamic shock.
...
\end{aligned}
$$
Note that we ignore particle escape toward far upstream.

<!--
![height:180px](2021-08-11-10-20-00.png)
-->

---

Probability that a particle remains in the vicinity of the shock after $l$ cycle: $P_{l} = (1 - P_{\rm esc})^{l} = \left( 1 - \frac{4 V_2}{c} \right)^{l}$.
...
- The canonical spectrum $q = 2$ is close to (but not exactly the same as) observed cosmic ray spectral index when taken into account a spectral steepening (via diffusive escape) during propagation from the source to the observer.
...
The first-order particle acceleration at shock waves described under the diffusion approximation is also known as diffusive shock acceleration (DSA).
...
Note that the escape time may also be estimated by
$$
\tau_{\rm esc} \sim \frac{1}{V_2} \left( \frac{\kappa_1}{V_1} + \frac{\kappa_2}{V_2} \right),
$$
which gives the spectral index
$$
p = \frac{\tau_{\rm acc}}{\tau_{\rm esc}} + 1 = \frac{V_1 + 2 V_2}{V_1 - V_2}.
$$
We see that the spectral index is consistent with that obtained previously with the single particle picture.

---

### Maximum energy

The acceleration time explicitly depends on the diffusion coefficient, which is not easily be estimated.
...
![height:440px center](figures/snr-shock.png)

---

### Application to Solar Corona (Ground Level Enhancement)
...
![height:440px center](figures/gle-coronal-shock.png)
...
![height:500px center](figures/hillas-diagram.png)
...
## Pitch-angle Scattering and Diffusion-Convection Equation
...
![height:350px center](figures/circularly-polarized-wave.png)

---

## Pitch-angle Scattering: Cyclotron Resonance
...
## Pitch-angle Scattering: Constants of Motion
...
![height:350px center](figures/scattering1.png)

---

## Pitch-angle Scattering: Diffusion in Momentum Space

<!-- _header: Ref: Terasawa (1989), Karimabadi+ (1992) -->

![height:600px center](figures/scattering2.png)
...
## Diffusion-Convection Equation: Spatial Diffusion
...
## Diffusion-Convection Equation: Adiabatic Energy Gain
The momentum gain of particles in a finite flow divergence $\nabla \cdot \boldsymbol{V} \neq 0$
$$
...
We finally obtain the following diffusion-convection equation (dropping the sbuscript for $f_0$)
$$
\frac{\partial}{\partial t} f +
...
![height:550px center](figures/gcr-sunspot.png)
